...
exports.deleteDraftHandler = exports.updateDraftItemHandler = exports.getDraftByIdHandler = exports.getAllDraftsHandler = exports.publishDraftHandler = void 0;
...
// ================================================
const publishDraftHandler = async (req, res) => {
    try {
        const { id } = req.params;
        // const { visibility } = req.body; // 'public' or 'private'
        // Find the draft
        const draft = await Draft_1.default.findByPk(id);
        if (!draft) {
        ...
        const archiveItem = await Archive_1.default.create({
            title: draft.title,
...
// @access Private (Admin only)
// ================================================
const getAllDraftsHandler = async (req, res) => {
    try {
        const drafts = await Draft_1.default.findAll();
        res.status(200).json({
...
exports.getAllDraftsHandler = getAllDraftsHandler;
// ================================================
// @desc Get draft by ID
// @route GET /drafts/:id
// @access Private (Admin only)
// ================================================
const getDraftByIdHandler = async (req, res) => {
    try {
        const { id } = req.params;
        const draft = await Draft_1.default.findByPk(id);
        if (!draft) {
...
exports.getDraftByIdHandler = getDraftByIdHandler;
// ================================================
...
//     }
// }
const updateDraftItemHandler = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, CategoryId, visibility } = req.body;
        // Find the draft item
        const draftItem = await Draft_1.default.findByPk(id);
        if (!draftItem) {
            return res.status(404).json({ message: 'Draft item not found' });
        }
        ...
        if (req.file) {
            ...
            const cloudinaryResult = await new Promise((resolve, reject) => {
                const folder = mediaType === "image" ? "archive-items/images"
