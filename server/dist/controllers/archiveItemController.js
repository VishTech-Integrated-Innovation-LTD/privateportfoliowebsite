...
exports.deleteArchiveItemHandler = exports.updateArchiveItemHandler = exports.getArchiveItemByIdHandler = exports.getArchiveItemsHandler = exports.createArchiveItemHandler = void 0;
...
const createArchiveItemHandler = async (req, res) => {
    try {
        ...
        // After getting title
        const existingItem = await Archive_1.default.findOne({ where: { title } });
        if (visibility === 'private') {
            const existingDraft = await Draft_1.default.findOne({ where: { title } });
            if (existingDraft) {
...
const getArchiveItemsHandler = async (req, res) => {
    try {
        ...
        cache_1.default.set(cacheKey, archives);
...
//     }
// }
const getArchiveItemByIdHandler = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.getArchiveItemByIdHandler = getArchiveItemByIdHandler;
// ================================================
...
//     }
// }
const updateArchiveItemHandler = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, CategoryId, visibility, isOnTheMainPage, addCollectionIds = [], // NEW: array of collection IDs to add
            ...
            if (archiveItem.cloudServiceUrl) {
                try {
                    ...
                    const resourceType = archiveItem.mediaType === 'video' ? 'video'
                        : archiveItem.mediaType === 'image' ? 'image'
                            : 'raw';
                    await cloudinaryConfig_1.default.uploader.destroy(publicId, { resource_type: resourceType });
                }
                catch (cloudinaryError) {
                    console.error('Error deleting old file from Cloudinary:', cloudinaryError);
                    // Continue with upload even if deletion fails
        ...
        if (visibility === 'private' && archiveItem.visibility === 'public') {
            ...
            const draftItem = await Draft_1.default.create({
                title: title || archiveItem.title,
                description: description || archiveItem.description,
                CategoryId: CategoryId || archiveItem.CategoryId,
                mediaType,
        ...
        if (addCollectionIds.length > 0) {
            const collectionsToAdd = await Collection_1.default.findAll({
        ...
        const updatedItem = await Archive_1.default.findByPk(id, {
            include: [{ model: Collection_1.default, as: 'Collections' }],
        });
        res.status(200).json({
            message: 'Archive item updated successfully',
            item: updatedItem,
...
exports.updateArchiveItemHandler = updateArchiveItemHandler;
// ================================================
